...
* **1. 「話した内容を覚えている」安心感と文脈理解**
  会話から「明日」「木曜日」などの相対的な日時をGeminiが計算しカレンダーへ自動登録。予定当日指定時間以降に「面談どうだった？」と**能動的にフォローアップ**を投げます。
...
* **4. APIエラーを防ぐ自動フォールバック構成**
  メインモデル（`gemini-3.5-flash`）のエラー時、即座に軽量モデル（第1フォールバック（予備）: `gemini-3.1-flash-lite`（軽量・高速）⇒第2フォールバック（バックアップ）: `gemini-2.5-flash`（安定モデル））へシームレスに切り替える機構を実装し、高い可用性を担保しています。
