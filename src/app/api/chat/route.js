...
export async function POST(req) {
  try {
    const { message, chatHistory = [], character = "clara", userName = "ユーザー", todaySchedules = [] } = await req.json();
...
- ユーザーのネガティブな感情（悲しみ、怒り、自己嫌悪、疲労など）を無理やり前向きに言い換えたり、「明日は良い日になる」「休めば大丈夫」といった薄っぺらい解決策や綺麗事（おためごかし）は絶対に言わないでください。
...
`;
    ...
    // Map chatHistory to Gemini Content format (must alternate strictly user -> model -> user)
    const contents = [
    ...
    // Call Gemini API with Fallback Chain to prevent 503 Service Overloads
    let replyText = "";
    try {
      ...
      console.warn("Primary gemini-3.5-flash failed or busy. Trying fallback gemini-3.1-flash-lite...", primaryErr.message);
      try {
        ...
        console.warn("Fallback gemini-3.1-flash-lite failed or busy. Trying backup gemini-2.5-flash...", fallbackErr.message);
