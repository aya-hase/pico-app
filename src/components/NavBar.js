...
export default function NavBar() {
  ...
  const navItems = [
    {
      ...
      icon: (active) => (
        <svg
          ...
          strokeWidth={1.5}
          stroke="currentColor"
          className={`w-6 h-6 transition-all ${active ? "scale-110 stroke-indigo-600 fill-indigo-50" : "stroke-slate-400"}`}
        >
          <path
            strokeLinecap="round"
  ...
  return (
    <nav className="border-t border-slate-100 bg-white/80 backdrop-blur-md px-3 py-2 pb-5 md:pb-3 flex justify-between items-center relative z-20">
      {navItems.map((item) => {
